...
###### SHELL EXPANSIONS ################################################################################################
#
# Expansions are a mechanism to calculate arithmetical operations, to save results of command's executions and so on.
...
# Command substitution is performed when a command is enclosed by  ``  or $().
...
#
# In bash we are free to do any arithmetic operations. But the expression must be enclosed by $(( )). The format for
# arithmetic expansions is:
#
#     result=$(( ((10 + 5*3) - 7) / 2 ))
#     echo $result # 9
#
...
###### THE CHALLENGE: Exercise 5 of 11 #################################################################################
#
# Create a file named expansions.bash.
# Your script should multiply the first positional parameter with the sum of
# the second and third parameter using arithmetic expansion. Save the result to the variable, $result e.g.
#
...
#please note {a,b,c} works but {a, b, c} does not works whitespaces are not allowed between values
echo project-$result/{dest/{index,util}.js,src/{index,util}.js,test/{index,util}.js}
